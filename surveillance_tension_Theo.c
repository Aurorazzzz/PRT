...
void setup(void)
{
    ...
    for (int i = 0; i < NbIteration; ++i)
    {
        ...
        // === Reproduction de detection_phase_charge_decharge.m ===
        ...
        tampon_charge_decharge[0] = courant_simulation;
        ...
        int etat_precedent_local = etat_precedent;

        // état = 0 --> charge
        // état = 1 --> décharge
        if (moyenne_charge_decharge > 0.1f && etat_precedent_local == 0) {
            // on était en charge, on passe en décharge
            etat = 1;
        } else if (moyenne_charge_decharge < -1.0f && etat_precedent_local == 1) {
            // on était en décharge, on passe en charge
            etat = 0;
        } else {
            etat = etat_precedent_local;
        }

        // mise à jour de etat_precedent (comme en fin de fonction MATLAB)
        etat_precedent = etat;
    ...
    Ecriture_result(vecteur_U, NbIteration, "TENSION_MODEL_raspi_result");
